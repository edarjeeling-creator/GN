...
const Login = () => {
  ...
  return (
    <div className="min-h-screen w-full flex relative overflow-hidden bg-[#05070c]">
      {/* Dynamic Background */}
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-brand-950/40 via-slate-950 to-[#030508]"></div>
        <motion.div 
          animate={{ x: [-15, 15, -15], y: [-15, 15, -15], opacity: [0.2, 0.4, 0.2] }}
          transition={{ repeat: Infinity, duration: 8, ease: "easeInOut" }}
          className="absolute top-1/4 left-1/4 w-80 h-80 bg-brand-500/10 rounded-full blur-3xl mix-blend-screen"
        />
        <motion.div 
          animate={{ x: [15, -15, 15], y: [15, -15, 15], opacity: [0.2, 0.4, 0.2] }}
          transition={{ repeat: Infinity, duration: 10, ease: "easeInOut", delay: 1 }}
          className="absolute bottom-1/4 right-1/4 w-[24rem] h-[24rem] bg-indigo-500/10 rounded-full blur-3xl mix-blend-screen"
        />
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center items-center z-10 px-4 sm:px-6 lg:px-8 relative">
        ...
        >
          <Card className="border-white/5 bg-slate-950/40 backdrop-blur-xl shadow-2xl p-8 sm:p-10 !rounded-[2rem] glass-card">
            ...
            <AnimatePresence mode="wait">
              {error && (
                <motion.div 
                  ...
                  className="mb-6"
                >
                  <div className="bg-red-500/10 border border-red-500/30 text-red-200 px-4 py-3 rounded-xl text-sm text-center font-medium backdrop-blur-sm">
                    {error}
                  </div>
                </motion.div>
              )}
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="bg-slate-950/60 border-white/5 text-white placeholder:text-slate-600 focus-visible:ring-brand-500 focus-visible:border-brand-500/50 h-12 rounded-xl transition-all"
                  required 
                />
              </div>
              ...
              <div className="pt-2">
                <Button 
                  type="submit" 
                  className="w-full h-12 text-base font-semibold bg-brand-600 hover:bg-brand-500 text-white shadow-[0_4px_20px_rgba(37,99,235,0.25)] hover:shadow-[0_4px_25px_rgba(37,99,235,0.4)] transition-all rounded-xl"
                  disabled={loading}
                >
                  {loading ? (
