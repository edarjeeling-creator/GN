...
BEGIN
  ...
  IF v_user IS NOT NULL THEN
    SELECT row_to_json(p) INTO v_profile FROM public.profiles p WHERE id = (v_user->>'id')::uuid;
  END IF;
  
  RETURN json_build_object('user', v_user, 'identity', v_identity, 'profile', v_profile);
END;
$$;
