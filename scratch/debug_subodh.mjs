...
async function run() {
  const { data, error } = await supabase.rpc('debug_get_user', { p_email: 'subodh@gyanodayniketan.cloud' });
  console.log("Subodh Identity:", JSON.stringify(data?.identity, null, 2));
}
